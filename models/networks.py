...
class MutDiscriminator(nn.Module):
    """Defines a MutPatchGAN discriminator"""
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d):
        ...
        Parameters:
            ...
            norm_layer      -- normalization layer
        """
        super(MutDiscriminator, self).__init__()
        if type(norm_layer) == functools.partial:  # no need to use bias as BatchNorm2d has affine parameters
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
        ...
        self.d1 = nn.Sequential(nn.Conv2d(ndf * nf_mult_prev, 1, kernel_size=3, stride=1, padding=1, bias=use_bias))
        sequence = [
            nn.Conv2d(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=kw, stride=2, padding=padw, bias=use_bias),
            norm_layer(ndf * nf_mult),
            nn.LeakyReLU(0.2, True)
        ...
        nf_mult_prev = nf_mult
        nf_mult = min(2 ** 2, 8)
        self.d2 = nn.Sequential(nn.Conv2d(ndf * nf_mult_prev, 1, kernel_size=3, stride=1, padding=1, bias=use_bias))
        sequence = [
            nn.Conv2d(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=kw, stride=2, padding=padw, bias=use_bias),
            norm_layer(ndf * nf_mult),
            nn.LeakyReLU(0.2, True)
        ]
        self.model3 = nn.Sequential(*sequence)
        self.models.append(self.model3)

        nf_mult_prev = nf_mult
        nf_mult = min(2 ** n_layers, 8)
        self.d3 = nn.Sequential(nn.Conv2d(ndf * nf_mult_prev, 1, kernel_size=3, stride=1, padding=1, bias=use_bias))
        sequence = [
            nn.Conv2d(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=kw, stride=1, padding=padw, bias=use_bias),
            norm_layer(ndf * nf_mult),
            nn.LeakyReLU(0.2, True)
        ...
        sequence += [nn.Conv2d(ndf * nf_mult, 1, kernel_size=kw, stride=1, padding=padw)]  # output 1 channel prediction map
        # self.model = nn.Sequential(*sequence)
        self.model4 = nn.Sequential(*sequence)
        self.models.append(self.model4)

        self.discriminators.extend([self.d1, self.d2,self.d3])
        # print(self.model)
        # print('cycle_gan: ',self.model)
    ...
    def forward(self, input):
        """Standard forward."""
        outputs = []
        input = self.models[0](input)
        for i in range(1,len(self.models)-1):
            model = self.models[i]
            output = self.discriminators[i-1](input)
            outputs.append(output)
            input = model(input)
        output = self.models[-1](input)
        outputs.append(output)
        return outputs


class CssDiscriminator(nn.Module):
    """Defines a MutPatchGAN discriminator"""

    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d,output_nc=3):
        """Construct a PatchGAN discriminator
        ...
        Parameters:
            ...
            norm_layer      -- normalization layer
        """
        super(CssDiscriminator, self).__init__()
        if type(norm_layer) == functools.partial:  # no need to use bias as BatchNorm2d has affine parameters
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
        ...
        self.d4 = nn.Conv2d(ndf*nf_mult,1,kernel_size=kw,stride=1,padding=padw)
        self.discriminators.extend([self.d1, self.d2,self.d3,self.d4])

        classifier_list = [
            nn.Conv2d(ndf * nf_mult,256,kernel_size=kw,stride=1,padding=padw),
            nn.LeakyReLU(0.01),
            nn.Conv2d(256, 128, kernel_size=4, stride=2, padding=padw),
            nn.LeakyReLU(0.01),
            nn.Conv2d(128, 64, kernel_size=5, stride=2, padding=padw),
            nn.LeakyReLU(0.01),
            nn.Conv2d(64, 32, kernel_size=3, stride=2, padding=padw),
            nn.LeakyReLU(0.01),
            nn.Conv2d(32, 16, kernel_size=2, stride=2, padding=padw),
            nn.LeakyReLU(0.01),
            nn.Conv2d(16, output_nc, kernel_size=2, stride=2),
            nn.LeakyReLU(0.01),
        ]
